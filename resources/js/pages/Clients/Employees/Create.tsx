...
export default function Create({ employee }: { employee: any }) {
    ...
    const actionButton = employee?.id ? (
        <>
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: employee?.id ? 'Edit' : 'Create',
            href: employee?.id ? `/employees/${employee.id}/edit` : '/tenants/create',
        },
    ];

    const { data, setData, post, put, processing, errors } = useForm({
        organization_name: employee?.tenant_name || '',
        organization_domain: employee?.id || '',
        organization_user_name: employee?.tenant_contact_name || '',
        organization_user_mobile: employee?.tenant_contact_number || '',
        organization_user_email: employee?.tenancy_db_email || '',
        organization_valid_from: employee?.valid_from ? new Date(employee.valid_from) : null,
        organization_valid_till: employee?.valid_till ? new Date(employee.valid_till) : null,
        organization_license_user_count: employee?.license_user_count || '',
        organization_timezone: employee?.base_timezone || '',
        organization_logo: null as File | null,
        logo_preview: employee?.tenant_logo || null,
    });
    ...
    return (
        <>
            <AppLayout breadcrumbs={breadcrumbs} actionButton={actionButton}>
                <Head title={`${employee?.id ? 'Edit' : 'Create'} Employees`} />
